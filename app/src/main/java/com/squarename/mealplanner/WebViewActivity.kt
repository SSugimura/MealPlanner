...
import com.squarename.mealplanner.rmethods.testTask
...
class WebViewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bookmark_button.setOnClickListener(View.OnClickListener{
            val task = RealmMethod().readSerch(true, webview.title, intent.getStringExtra(("url")))
            if(task.isEmpty()) {
                RealmMethod().create(true, webview.title, intent.getStringExtra(("url")))
                showToast("ブックマークしました")
            }else{
                RealmMethod().deleteTitle(true,webview.title)
